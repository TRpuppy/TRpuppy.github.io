...
我不喜欢换两行。觉得行距太大了。所以我一般会输入`shift+enter`换行。
...
为什么不喜欢它的上下标呢，因为本来Latex就有上下标，且md显示的上下标还会有显示问题。例如`x~1~^2^`显示出来是x~1~^2^,而用Latex的`$x_1^2$`——$x_1^2$就会漂亮很多。
...
还可以在圆括号里面使用双引号为链接添加别名。
形如`[name](URL "other name")`。呈现效果：[百度一下](www.baidu.com "一个垃圾搜索引擎")
