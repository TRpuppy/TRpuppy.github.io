...
体渲染有两种方式进行。一种方式和光线追踪的思路很相似，名叫**光线投影**。它对每个像素点发一条光线，通过计算每条光线与体内的元素相交得到的颜色和强度来生成图像。我们不过度深入这里面的物理细节，而只关心算法思路。大致算法步骤如下所示：

![image-20251222202510684](image-20251222202510684.png)

另一种方法叫体积泼溅。这种方法将体素投影到像素平面上，每个体素的投影是以它的投影坐标为中心的一个插值权重核。在渲染图像的时候，按照体素之间的遮挡关系(可以通过按深度排序来进行顺序泼溅来实现)进行计算。

最终图像平面上每个像素的值是所有相关体素贡献的加权和。
